...
export function MealSeekerContent() {
  ...
  const handleFridgeConfirm = (newIngredients: string[]) => {
    const uniqueNewItems = newIngredients.filter(
    ...
    if (uniqueNewItems.length > 0) {
      ...
      if (!streakData) startStreak();
      else updateStreak();

      setIngredients(prev => [...prev, ...uniqueNewItems.map(i => i.toLowerCase())]);
      toast.success(`Added ${uniqueNewItems.length} items from your fridge!`);
    }
    ...
    <main className='xl:grid xl:grid-cols-3 flex flex-col min-h-screen'>
      <section className="relative xl:sticky xl:top-0 xl:h-screen xl:bg-[url('/Background.png')] xl:bg-no-repeat xl:bg-center xl:bg-cover p-6 sm:p-8 md:p-10">
        ...
        <div className='font-merriweather text-2xl mb-6'>
          <h3 className='flex justify-start items-center'>Your streak: <Flame />{streak}</h3>
        </div>
        <div className='relative border border-[#828181] rounded-md bg-white flex flex-row items-center w-fit mb-6'>
          ...
          <Button 
            ...
            variant='ghost'
            className='px-3 font-light text-xl bg-[#004E89] text-[#EFEFD0] cursor-pointer hover:bg-[#1A659E] mx-0.5 my-0.5'
            title='Scan Fridge'
          >
            <Camera />
          </Button>
      ...
      {/* NEW FRIDGE MODAL */}
        ...
        <FridgeScanner isOpen={activeModal} setIsOpen={setActiveModal} onConfirm={handleFridgeConfirm}/>
    </main>
  )
};